...
def bsGetGames():
    return [ThrowAwayGame]

class ThrowAwayGame(bs.TeamGameActivity):
    ...
    def getDescription(cls,sessionType):
        return ('No bombs no punches only muscles. \n'
				'Throw away your enemies from the cliff !\n'
    ...
    def getSupportedMaps(cls,sessionType):
        return ['Bridgit','Big G','Roundabout','Monkey Face','Zigzag','The Pad','Doom Shroom','Tip Top','Crag Castle','Rampage']

    @classmethod
    def getSettings(cls,sessionType):
        return [("Throw to Win Per Player",{'minValue':1,'default':5,'increment':1}),
    ...
    def getInstanceDescription(self):
        return ('Throw ${ARG1} of your enemies from the cliff.',self._scoreToWin)

    def getInstanceScoreBoardDescription(self):
        return ('Throw ${ARG1} enemies from the cliff',self._scoreToWin)
    ...
    def onBegin(self):
        ...
        if len(self.teams) > 0:
            self._scoreToWin = self.settings['Throw to Win Per Player'] * max(1,max(len(t.players) for t in self.teams))
        else: self._scoreToWin = self.settings['Throw to Win Per Player']
    ...
    def handleMessage(self,m):
        ...
        if isinstance(m,bs.PlayerSpazDeathMessage):
            ...
            if m.spaz.lastAttackedType == ('pickedUp', 'default'):
